...
class SlackOAuthService:
    ...
    def exchange_code_for_token(
        ...
        try:
            ...
            print(f"[SLACK-DEBUG] exchange_code_for_token called")
            print(f"[SLACK-DEBUG] client_id={self.client_id}")
            print(f"[SLACK-DEBUG] client_secret={'***' + self.client_secret[-4:] if self.client_secret and len(self.client_secret) > 4 else '???'}")
            print(f"[SLACK-DEBUG] redirect_uri={self.redirect_uri}")
            print(f"[SLACK-DEBUG] code={code[:15]}...")
            ...
            print(f"[SLACK-DEBUG] oauth_v2_access response ok={response.get('ok')}")
            
            if not response["ok"]:
                error = response.get("error", "Unknown error")
                logger.error(f"Slack OAuth token exchange failed: {error}")
                print(f"[SLACK-DEBUG] Slack returned error: {error}")
            ...
            integration_data = {
                ...
                "bot_access_token": encrypted_bot_token,
            ...
            error_msg = e.response.get("error", str(e)) if hasattr(e, 'response') and e.response else str(e)
            logger.error(f"Slack API error during token exchange: {error_msg}")
            print(f"[SLACK-DEBUG] SlackApiError: {error_msg}")
            print(f"[SLACK-DEBUG] Full SlackApiError: {e}")
            return False, None, f"Slack API error: {error_msg}"
        except Exception as e:
            logger.error(f"Unexpected error during token exchange: {type(e).__name__}: {e}")
            print(f"[SLACK-DEBUG] Exception type={type(e).__name__}: {e}")
            import traceback
            traceback.print_exc()
