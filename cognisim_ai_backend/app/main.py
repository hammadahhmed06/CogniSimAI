...
from app.api.routes.slack import (
    events_router as slack_events_router,
    commands_router as slack_commands_router,
    interactive_router as slack_interactive_router,
)
# GitHub App integration (installations, webhooks, repos, Projects v2)
from app.api.routes.github.app import router as github_router
# Jira OAuth only (connect/disconnect)
from app.api.routes.jira import (
    oauth_router as jira_oauth_router,
    projects_router as jira_projects_router,
    boards_router as jira_boards_router,
    issues_router as jira_issues_router,
    sync_router as jira_sync_router
)
...
from app.api.routes.historical_metrics import router as historical_metrics_router
app.include_router(historical_metrics_router)

# Jira integration routes (OAuth, projects, boards, issues, sync, settings, webhooks, conflicts)
app.include_router(jira_oauth_router)
app.include_router(jira_projects_router)
app.include_router(jira_boards_router)
app.include_router(jira_issues_router)
app.include_router(jira_sync_router)
from app.api.routes.jira.settings import router as jira_settings_router
app.include_router(jira_settings_router)
from app.api.routes.jira.conflicts import router as jira_conflicts_router
app.include_router(jira_conflicts_router)
...
from app.api.routes.agent_config import router as agent_config_router
...
# Initialise the notification dispatcher so it registers event-bus handlers
from app.services.notifications import notification_service as _ns  # noqa: F401
