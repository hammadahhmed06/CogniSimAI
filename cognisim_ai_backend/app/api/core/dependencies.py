...
class _SupabaseLazy:
    ...
    def _init(self) -> Client:
        ...
        # Remove trailing slash from SUPABASE_URL to avoid double slash in API calls
...
async def get_optional_user(credentials: Optional[HTTPAuthorizationCredentials] = Depends(optional_bearer_scheme)) -> Optional[UserModel]:
    if not credentials:
        return None
    token: str = credentials.credentials
    try:
