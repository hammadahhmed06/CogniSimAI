    ...
    try:
        ...
        if not member:
            raise HTTPException(
                ...
                detail=f"You are not a member of workspace {workspace_id}"
            )
        
        user_role = member.get("role")
        logger.info(f"User {current_user.id} has role '{user_role}' in workspace {workspace_id}")
        
        if user_role not in ["admin", "owner"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=f"Admin or owner access required for workspace integrations. Your current role: {user_role}. Please ask a workspace admin/owner to set up integrations."
            )
        return True
    except HTTPException:
        raise
...
@router.get(
    ...
    description="Get Slack integration status for workspace"
)
async def get_slack_integration(
    ...
    try:
        ...
        integration_data = getattr(integration_res, "data", [])
        if not integration_data:
            # Return disconnected status when no integration exists
        ...
        integration = integration_data[0]
        
        # Get workspace name
        workspace_res = (
            supabase.table("workspaces")
            .select("name")
            .eq("id", workspace_id_str)
            .single()
            .execute()
        )
        workspace_name = getattr(workspace_res.data, "name", None) if workspace_res.data else None
        
        # Get installer email
        installed_by_email = None
        if integration.get("installed_by"):
            user_res = (
                supabase.table("users")
                .select("email")
                .eq("id", str(integration["installed_by"]))
                .single()
                .execute()
            )
            installed_by_email = getattr(user_res.data, "email", None) if user_res.data else None
        
        return SlackIntegrationStatusResponse(
            is_connected=True,
            workspace_id=workspace_id,
            workspace_name=workspace_name,
            slack_workspace_name=integration.get("slack_workspace_name"),
            default_channel_name=integration.get("default_channel_name"),
            ...
            installed_by_email=installed_by_email,
        ...
        logger.error(f"Failed to get Slack integration status: {e}")
        import traceback
...
async def create_slack_integration(
    ...
    try:
        ...
        integration_data = {
            ...
            "bot_access_token": encrypted_token,
        ...
        try:
            ...
            event_bus.emit(EventType.INTEGRATION_CONNECTED, {
        ...
        integration["bot_access_token"] = "***ENCRYPTED***"
...
async def delete_slack_integration(
    ...
    try:
        ...
        # Emit integration disconnected event
        try:
            from app.services.events import event_bus, EventType
            event_bus.emit(EventType.INTEGRATION_DISCONNECTED, {
                "integration_type": "slack",
                "workspace_id": workspace_id_str,
                "user_id": current_user.id,
            })
...
async def test_slack_connection(
    ...
    try:
        ...
        integration_res = (
            supabase.table("slack_integrations")
            .select("bot_access_token, is_active")
            .eq("workspace_id", workspace_id_str)
            .single()
            .execute()
        )
...
async def slack_oauth_callback(
    ...
    - OAuth state validation provides CSRF protection
    - User must be authenticated when initiating OAuth (state creation)
    - State contains user_id which is validated during callback
    """
    try:
        # Check for errors from Slack
        oauth_service = get_slack_oauth_service()
        base_frontend_url = oauth_service.settings.FRONTEND_URL.rstrip("/")

        if error:
            logger.error(f"❌ Slack OAuth error: {error}")
            logger.error(f"❌ Full callback URL: {code if code else 'no code'}, State: {state if state else 'no state'}")
            return RedirectResponse(url=f"{base_frontend_url}/dashboard/integrations?slack_error={error}")
        
        # Validate required parameters
        if not code or not state:
            logger.error("❌ Slack callback missing required parameters")
            logger.error(f"❌ Received - code: {bool(code)}, state: {bool(state)}")
            return RedirectResponse(url=f"{base_frontend_url}/dashboard/integrations?slack_error=missing_parameters")
        
        # Validate state directly (no redundant service call)
        logger.info(f"🔄 Slack OAuth Callback - Code: {code[:10]}..., State: {state[:10]}...")
        
        try:
            # Get and validate state record in one step
            state_record_res = (
            ...
            state_record = getattr(state_record_res, "data", None)
            
            if not state_record:
                logger.error("❌ Invalid Slack OAuth state - not found or already used")
                return RedirectResponse(url=f"{base_frontend_url}/dashboard/integrations?slack_error=invalid_state")
            
            # Check expiration
            from datetime import datetime, timezone
            expires_at_str = state_record.get("expires_at", "")
            if expires_at_str:
                expires_at = datetime.fromisoformat(expires_at_str.replace("Z", "+00:00"))
                if datetime.now(timezone.utc) > expires_at:
                    logger.error("❌ OAuth state expired")
                    return RedirectResponse(url=f"{base_frontend_url}/dashboard/integrations?slack_error=state_expired")
            
            # Extract validated data
            user_id_from_state = UUID(state_record["user_id"])
            workspace_id = UUID(state_record["workspace_id"])
            redirect_uri = state_record.get("redirect_uri")
            
            logger.info(f"✅ State validated - User: {user_id_from_state}, Workspace: {workspace_id}")
            ...
            logger.error(f"❌ Failed to validate state: {state_err}")
            return RedirectResponse(url=f"{base_frontend_url}/dashboard/integrations?slack_error=invalid_state")
        
        logger.info(f"🔑 Exchanging Slack code for tokens...")
        print(f"[SLACK-DEBUG] Exchanging code for tokens, workspace={workspace_id}")
        print(f"[SLACK-DEBUG] redirect_uri={oauth_service.redirect_uri}")
        ...
        if not success or not integration_data:
            ...
            print(f"[SLACK-DEBUG] ❌ Token exchange FAILED: {error_msg}")
            import urllib.parse as _up
            safe_err = _up.quote(str(error_msg or 'unknown')[:200])
            return RedirectResponse(url=f"{base_frontend_url}/dashboard/integrations?slack_error=token_exchange_failed&detail={safe_err}")
        
        print(f"[SLACK-DEBUG] Token exchange succeeded, saving to DB...")
        print(f"[SLACK-DEBUG] integration_data keys: {list(integration_data.keys())}")
        ...
        if getattr(existing_res, "data", []):
            ...
            print(f"[SLACK-DEBUG] Updating existing integration...")
            update_res = (
                supabase.table("slack_integrations")
            ...
            print(f"[SLACK-DEBUG] Creating new integration...")
            integration_data["installed_by"] = str(user_id_from_state)
            insert_res = (
                supabase.table("slack_integrations")
                .insert(integration_data)
                .execute()
        ...
        print(f"[SLACK-DEBUG] DB result: {bool(integration)}")
        if not integration:
        ...
        try:
            ...
            event_bus.emit(EventType.INTEGRATION_CONNECTED, {
                ...
                "user_id": str(user_id_from_state) if user_id_from_state else None,
            })
        except Exception:
            pass

        # Redirect to frontend with success
        final_redirect = redirect_uri or f"{base_frontend_url}/dashboard/integrations?slack_success=true"
        ...
        print(f"[SLACK-DEBUG] ❌ OUTER EXCEPTION: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        try:
            fallback_frontend = get_slack_oauth_service().settings.FRONTEND_URL.rstrip("/")
        except Exception:
            fallback_frontend = ""
        import urllib.parse as _up2
        safe_detail = _up2.quote(f"{type(e).__name__}: {str(e)[:200]}")
        return RedirectResponse(url=f"{fallback_frontend}/dashboard/integrations?slack_error=unknown&detail={safe_detail}")
