...
def sync_user_to_users_table(user_id: str, email: str = None, full_name: str = None) -> bool:
    """Sync a user from auth.users to the public.users table.
    
    This is required because team_members.user_id has a FK constraint to users.id.
    Uses auth.admin.get_user_by_id() to get actual user data from Supabase Auth.
    
    Returns True if user exists (or was created), False if sync failed.
    """
    try:
        ...
        # Step 2: Get user info from auth.users via admin API
        user_data = {"id": user_id}
        
        # Try to get info from auth.users
        try:
            auth_user_result = supabase.auth.admin.get_user_by_id(user_id)
            if auth_user_result and auth_user_result.user:
                ...
                user_data["full_name"] = metadata.get("full_name") or metadata.get("name") or (auth_user.email.split("@")[0] if auth_user.email else "User")
                if metadata.get("avatar_url"):
                    user_data["avatar_url"] = metadata["avatar_url"]
                logger.info(f"Got auth user data for {user_id}: email={auth_user.email}")
        except Exception as auth_e:
            logger.warning(f"Could not get auth user {user_id}: {auth_e}")
            # Fall back to provided email/name
            if email:
                user_data["email"] = email
                user_data["full_name"] = full_name or email.split("@")[0]
        
        # Step 3: Try to get info from user_profiles as fallback
        if "email" not in user_data or not user_data.get("email"):
            try:
                profile = supabase.table("user_profiles").select("email,full_name,avatar_url").eq("id", user_id).maybe_single().execute()
                ...
                if profile_data:
                    ...
                    if profile_data.get("avatar_url"):
                        user_data["avatar_url"] = profile_data["avatar_url"]
            except Exception as profile_e:
                logger.warning(f"Could not get user_profiles for {user_id}: {profile_e}")
        
        # Step 4: Insert user into users table
        logger.info(f"Syncing user {user_id} to users table with data: {user_data}")
        supabase.table("users").insert(user_data).execute()
        logger.info(f"Successfully synced user {user_id} to users table")
        return True
        
    except Exception as e:
        error_str = str(e)
        
        # Duplicate key means user exists (race condition - that's OK)
        if "23505" in error_str or "duplicate key" in error_str.lower():
            logger.debug(f"User {user_id} already exists (duplicate key)")
            return True
        
        logger.error(f"Failed to sync user {user_id} to users table: {error_str}")
        return False


def ensure_user_exists_by_id(user_id: UUID) -> bool:
    """Ensure a user exists in the users table by their ID.
    
    Wrapper around sync_user_to_users_table for backward compatibility.
    """
    return sync_user_to_users_table(str(user_id))
...
async def create_team(
    ...
    - Same user can create teams with same name in different workspaces
    - Creator automatically becomes team owner
    """
    user_id_str = str(current_user.id)
    workspace_id_str = str(wctx.workspace_id)
    team_name = body.name.strip()
    
    if not team_name:
        raise HTTPException(status_code=400, detail="Team name cannot be empty")
    
    # Step 1: Sync user to users table FIRST (required for FK constraint on team_members)
    # This must happen before any other operation to ensure the FK constraint is satisfied
    user_synced = sync_user_to_users_table(user_id_str, email=current_user.email)
    if not user_synced:
        logger.error(f"Failed to sync user {user_id_str} to users table")
        raise HTTPException(
            status_code=500,
            detail="Unable to set up your account. Please try again or contact support."
        )
    
    # Step 2: Check if team name already exists in this workspace
    try:
        ...
        if getattr(existing, "data", None):
            raise HTTPException(
                status_code=400, 
                detail=f"A team named '{team_name}' already exists in this workspace. Please choose a different name."
            )
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error checking existing team: {e}")
        # Continue - we'll catch duplicate key error later if needed
    ...
    try:
        ...
        logger.info(f"Team {team_id_str} created: {team_name}")
    except Exception as e:
        error_str = str(e)
        if "23505" in error_str or "duplicate key" in error_str.lower():
            raise HTTPException(
                status_code=400,
                detail=f"A team named '{team_name}' already exists. Please choose a different name."
            )
        logger.error(f"Failed to create team: {error_str}")
        raise HTTPException(status_code=500, detail="Failed to create team. Please try again.")
    
    # Step 4: Add creator as team owner
    try:
        ...
        logger.info(f"Added user {user_id_str} as owner of team {team_id_str}")
    except Exception as e:
        # Rollback: delete the team we just created
        logger.error(f"Failed to add team member: {e}")
        try:
            supabase.table("teams").delete().eq("id", team_id_str).execute()
            logger.info(f"Rolled back team creation for {team_id_str}")
        except Exception as rollback_error:
            logger.error(f"Failed to rollback team: {rollback_error}")
        
        error_str = str(e)
        if "23503" in error_str or "foreign key" in error_str.lower():
            raise HTTPException(
                status_code=500,
                detail="Unable to add you to the team. User sync may have failed. Please try again."
            )
        raise HTTPException(status_code=500, detail="Failed to create team. Please try again.")
...
async def cleanup_orphan_teams(
    current_user: UserModel = Depends(get_current_user),
    wctx: WorkspaceContext = Depends(get_workspace_context),
):
    """
    Delete teams in the workspace that have no members (orphan teams).
    This can happen if team creation partially fails.
    """
    workspace_id_str = str(wctx.workspace_id)
    
    # Find all teams in workspace
    ...
    for team in teams:
        ...
        members_res = supabase.table("team_members")\
            .select("id")\
            .eq("team_id", team_id)\
            .limit(1)\
            .execute()
        
        members = getattr(members_res, "data", []) or []
        
        if len(members) == 0:
            # Orphan team - delete it
            try:
                supabase.table("teams").delete().eq("id", team_id).execute()
                deleted.append({"id": team_id, "name": team_name})
                logger.info(f"Deleted orphan team: {team_name} ({team_id})")
            except Exception as e:
                logger.error(f"Failed to delete orphan team {team_id}: {e}")
    
    return {
        "message": f"Cleaned up {len(deleted)} orphan teams",
        "deleted": deleted
    }
...
async def list_members(team_id: UUID, ctx=Depends(team_role_required("viewer", "editor", "admin", "owner"))):
    ...
    # Hydrate members with profile data (best-effort)
    ...
    if user_ids:
        ...
        # Fallback to public.users if profile missing
        missing = [uid for uid in user_ids if uid not in profiles_by_id]
        if missing:
            try:
                ur = (
                ...
                for u in (getattr(ur, "data", []) or []):
                    uid = str(u.get("id"))
                    if uid and uid not in profiles_by_id:
                        profiles_by_id[uid] = u
            except Exception:
                pass
...
async def add_member(team_id: UUID, body: AddMemberRequest, ctx=Depends(team_role_required("admin", "owner"))):
    ...
    # Sync user to users table (required for FK constraint on team_members)
    user_synced = sync_user_to_users_table(str(body.user_id))
    if not user_synced:
        raise HTTPException(status_code=500, detail="Unable to add member. User sync failed.")
    ...
    try:
        ...
        error_str = str(e)
        if "23503" in error_str or "foreign key" in error_str.lower():
            raise HTTPException(status_code=500, detail="Unable to add member. User does not exist in system.")
        raise HTTPException(status_code=500, detail=f"Failed to add member: {error_str}")
...
async def add_members_batch(team_id: UUID, body: BatchAddMembersRequest, ctx=Depends(team_role_required("admin", "owner"))):
    ...
    for uid in body.users:
        try:
            ...
            if existing:
                continue
            # Sync user to users table (required for FK constraint)
            if not sync_user_to_users_table(str(uid)):
                errors.append({"user_id": str(uid), "error": "User sync failed"})
                continue
            supabase.table("team_members").insert({
                "id": str(uuid4()),
                "team_id": str(team_id),
...
async def invite_member(
    ...
    existing_user = None
    try:
        user_res = supabase.table("user_profiles").select("id,email").eq("email", body.email).execute()
    ...
    # Try to store invitation in database (table might not exist)
    invitation_stored = False
    try:
        invitation_data = {
            "id": str(uuid4()),
        ...
        invitation_stored = True
    except Exception as e:
        # Log but continue - we can still send email without storing invitation
        logger.warning(f"Could not store invitation in database: {e}")
    
    # Generate invitation link
    frontend_url = os.getenv("FRONTEND_URL", body.redirect or "http://localhost:5173")
    invite_link = f"{frontend_url}/accept-invite?token={invitation_token}&team={team_id}"
    
    # Get inviter name
    inviter_name = "A team member"
    try:
        inviter_profile = supabase.table("user_profiles").select("full_name,email").eq("id", str(current_user.id)).execute()
    ...
    email_error = None
    try:
        logger.info(f"Attempting to send invitation email to {body.email}")
        ...
        error_str = str(e)
        logger.error(f"Failed to send invitation email to {body.email}: {error_str}")
        email_error = error_str
    
    # Build response message
    if email_sent:
        message = f"Invitation sent to {body.email}"
    elif invitation_stored:
        message = f"Invitation created but email failed to send. You can share the invite link manually."
    else:
        message = f"Could not send invitation. Please share the invite link manually."
    
    return {
        "success": email_sent or invitation_stored,
        "message": message,
        "token": str(invitation_token),
        "invite_link": invite_link,
        "email_sent": email_sent,
        "email_error": email_error,
        "invitation_stored": invitation_stored,
        "expires_at": expires_at.isoformat()
    }
...
# ============= WORKSPACE USERS SEARCH FOR DIRECT TEAM MEMBER ADDITION =============
...
@router.get("/{team_id}/available-users", response_model=List[WorkspaceUserResponse])
async def list_available_users(
    ...
    """
    List workspace users who can be added to this team.
    Returns users in the same workspace, indicating if they're already team members.
    """
    try:
        # Use workspace_members as the source of truth (users who have joined the workspace)
        wm_res = (
            supabase.table("workspace_members")
            .select("user_id")
            .eq("workspace_id", str(wctx.workspace_id))
            .eq("status", "active")
            .execute()
        )
        wm_rows = getattr(wm_res, "data", []) or []
        workspace_user_ids = list({str(m.get("user_id")) for m in wm_rows if m.get("user_id")})

        if not workspace_user_ids:
            return []
        
        # Get current team members
        current_team_members_res = supabase.table("team_members")\
            .select("user_id")\
            .eq("team_id", str(team_id))\
            .execute()
        current_member_ids = set([m['user_id'] for m in (getattr(current_team_members_res, 'data', []) or [])])
        
        # Get user profiles (best-effort)
        profiles: list[dict] = []
        try:
            profiles_res = (
                supabase.table("user_profiles")
                .select("id, email, full_name, avatar_url")
                .in_("id", workspace_user_ids)
                .execute()
            )
        ...
        # Fallback to public.users for missing profiles
        profile_ids = {str(p.get('id')) for p in profiles if p.get('id')}
        missing = [uid for uid in workspace_user_ids if uid not in profile_ids]
        if missing:
            try:
                users_res = (
                    supabase.table("users")
                ...
                profiles.extend(getattr(users_res, 'data', []) or [])
            except Exception:
                pass
        ...
        if search:
            ...
            profiles = [
                p for p in profiles
                if (p.get('email', '').lower().find(search_lower) >= 0 or
                    (p.get('full_name') or '').lower().find(search_lower) >= 0)
            ]
        
        # Build response
        users: List[WorkspaceUserResponse] = []
        for p in profiles[:limit]:
            users.append(WorkspaceUserResponse(
                id=UUID(p['id']),
                email=p.get('email', ''),
                full_name=p.get('full_name'),
                avatar_url=p.get('avatar_url'),
                is_team_member=str(p['id']) in current_member_ids
            ))
        
        # Sort: non-members first, then by name
        users.sort(key=lambda u: (u.is_team_member, (u.full_name or u.email or '').lower()))
        
        return users
    except Exception as e:
        logger.error(f"Failed to list available users: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to list available users: {str(e)}")


class DirectAddMemberRequest(BaseModel):
    """Add a user directly to the team by their user ID"""
    user_id: UUID
    role: str = "viewer"


@router.post("/{team_id}/members/direct", response_model=TeamMember)
async def add_member_direct(
    team_id: UUID,
    body: DirectAddMemberRequest,
    current_user: UserModel = Depends(get_current_user),
    wctx: WorkspaceContext = Depends(get_workspace_context),
    ctx=Depends(team_role_required("admin", "owner"))
):
    """Add a workspace member directly to the team without sending an invitation.

    Requirement: user must have an active `workspace_members` row for the current workspace.
    """
    # Validate role
    assert_valid_role(body.role)
    ...
    wm_rows = getattr(wm, 'data', []) or []
    if not wm_rows or (wm_rows[0].get('status') != 'active'):
    ...
    existing_res = supabase.table("team_members")\
        .select("id")\
        .eq("team_id", str(team_id))\
        .eq("user_id", str(body.user_id))\
        .maybe_single()\
    ...
    if getattr(existing_res, 'data', None):
        raise HTTPException(status_code=400, detail="User is already a team member")
    
    # Ensure user exists in users table (required for FK constraint)
    ensure_user_exists_by_id(body.user_id)
    ...
    return TeamMember(id=mid, user_id=body.user_id, role=body.role, status="active")
