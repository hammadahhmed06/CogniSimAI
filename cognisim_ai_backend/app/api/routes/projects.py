...
def _project_visible_to_user(project_row: dict, workspace_id: UUID, user_id: UUID) -> bool:
    ...
    try:
        ...
        acc = (
            ...
            .select("id")
            .eq("project_id", str(project_row.get('id')))
...
def project_metrics_summary(project_id: UUID, ctx: WorkspaceContext = Depends(get_workspace_context), current_user: UserModel = Depends(get_current_user)):
    # Ensure project is in the active workspace and visible
    proj_res = supabase.table("projects").select("id,workspace_id,owner_id").eq("id", str(project_id)).maybe_single().execute()
    proj_row = getattr(proj_res, 'data', None)
    if not proj_row:
        raise HTTPException(status_code=404, detail="Project not found")
    if str(proj_row.get('workspace_id')) != str(ctx.workspace_id):
        raise HTTPException(status_code=403, detail="Cross-workspace access denied")
    if not _project_visible_to_user(proj_row, ctx.workspace_id, current_user.id):
        raise HTTPException(status_code=404, detail="Project not found")
    # Basic placeholder metrics derived from issues; refine later
    issues_res = supabase.table("issues").select("id,status,started_at,done_at,story_points").eq("project_id", str(project_id)).execute()
...
class ProjectUpdate(BaseModel):
    ...
    status: Optional[str] = Field(None, pattern=r"^(active|archived|completed)$")
...
def list_projects(q: Optional[str] = None, status: Optional[str] = None, ctx: WorkspaceContext = Depends(get_workspace_context), current_user: UserModel = Depends(get_current_user)):
    ...
    try:
        query = supabase.table("projects").select(
            "id,name,key,type,description,status,created_at,updated_at,archived_at,slug,workspace_id,owner_id,integration_id,jira_project_id,jira_project_key"
        ).eq("workspace_id", str(ctx.workspace_id))
        if status in {"active", "archived"}:
            query = query.eq("status", status)
        res = query.execute()
    except APIError as e:  # legacy schema missing 'type' or jira fields
        if 'type' in str(e) or 'integration_id' in str(e) or 'jira' in str(e).lower():
            try:
                query = supabase.table("projects").select("id,name,key,description,status,created_at,updated_at,archived_at,integration_id,jira_project_id,jira_project_key").eq("owner_id", str(current_user.id)).eq("workspace_id", str(ctx.workspace_id))
                if status in {"active", "archived"}:
                    query = query.eq("status", status)
                res = query.execute()
            except Exception:
...
def batch_project_stats(ids: str, ctx: WorkspaceContext = Depends(get_workspace_context), current_user: UserModel = Depends(get_current_user)):
    ...
    if not id_list:
        return {}
    # Ensure visibility: fetch candidate projects within workspace and filter by access
    allowed_res = supabase.table("projects").select("id,workspace_id,owner_id").in_("id", id_list).eq("workspace_id", str(ctx.workspace_id)).execute()
    allowed_rows = getattr(allowed_res, 'data', []) or []
    allowed_ids = {
        r.get('id')
        for r in allowed_rows
        if r.get('id') and _project_visible_to_user(r, ctx.workspace_id, current_user.id)
    }
    if not allowed_ids:
        return {}
...
def get_project_by_slug(slug: str, ctx: WorkspaceContext = Depends(get_workspace_context), current_user: UserModel = Depends(get_current_user)):
    res = supabase.table("projects").select("id,name,key,type,description,status,created_at,updated_at,archived_at,slug,workspace_id,owner_id").eq("slug", slug).eq("workspace_id", str(ctx.workspace_id)).maybe_single().execute()
...
def archive_project(project_id: UUID, ctx: WorkspaceContext = Depends(get_workspace_context), current_user: UserModel = Depends(get_current_user)):
    ...
    upd = supabase.table("projects").update({
        "status": "archived",
        "updated_at": datetime.utcnow().isoformat(),
        "archived_at": datetime.utcnow().isoformat()
    }).eq("id", str(project_id)).execute()
    updated_row = (getattr(upd, 'data', []) or [None])[0]
    if not updated_row:
    ...
    return Project(
        ...
        archived_at=updated_row.get('archived_at'),
        slug=updated_row.get('slug'),
        workspace_id=updated_row.get('workspace_id')
    )

@router.post("/{project_id}/complete", response_model=Project)
def complete_project(project_id: UUID, ctx: WorkspaceContext = Depends(get_workspace_context), current_user: UserModel = Depends(get_current_user)):
    # Fetch existing project
    existing = supabase.table("projects").select("id,name,key,type,description,status,created_at,updated_at,slug,workspace_id,completed_at,archived_at").eq("id", str(project_id)).eq("owner_id", str(current_user.id)).maybe_single().execute()
    row = getattr(existing, 'data', None)
    if not row:
        raise HTTPException(status_code=404, detail="Project not found")
    if str(row.get('workspace_id')) != str(ctx.workspace_id):
    ...
    # Mark as completed using status='completed' and completed_at timestamp
    upd = supabase.table("projects").update({
        "status": "completed",
    ...
    _log_project_activity(project_id, current_user.id, "complete")
    return Project(
        id=updated_row['id'],
        name=updated_row['name'],
        key=updated_row['key'],
...
def delete_project(project_id: UUID, ctx: WorkspaceContext = Depends(get_workspace_context), current_user: UserModel = Depends(get_current_user)):
    """Permanently delete a project and all associated data."""
    # Fetch existing project
    existing = supabase.table("projects").select("id,name,workspace_id,owner_id").eq("id", str(project_id)).eq("owner_id", str(current_user.id)).maybe_single().execute()
    row = getattr(existing, 'data', None)
    if not row:
        raise HTTPException(status_code=404, detail="Project not found")
    if str(row.get('workspace_id')) != str(ctx.workspace_id):
    ...
    try:
        ...
        supabase.table("project_activity").delete().eq("project_id", str(project_id)).execute()
        
        # Delete project team access
        supabase.table("project_team_access").delete().eq("project_id", str(project_id)).execute()
        
        # Delete sprints
        supabase.table("sprints").delete().eq("project_id", str(project_id)).execute()
        
        # Delete issues (this will cascade to comments, dependencies, etc. if DB constraints are set)
        supabase.table("issues").delete().eq("project_id", str(project_id)).execute()
        
        # Finally, delete the project itself
        delete_result = supabase.table("projects").delete().eq("id", str(project_id)).execute()
        
        if not getattr(delete_result, 'data', None):
...
def start_sprint(project_id: UUID, sprint_id: UUID, current_user: UserModel = Depends(get_current_user)):
    ...
    try:
        ...
        event_bus.emit(EventType.SPRINT_STARTED, {
            "sprint_id": str(sprint_id),
            "sprint_name": data[0].get("name", "Sprint"),
            "project_id": str(project_id),
            "user_id": current_user.id,
            "workspace_id": str(getattr(proj, 'data', {}).get('workspace_id', '')),
        })
    except Exception:
        pass
    return _sprint_from_row(data[0])
...
def complete_sprint(project_id: UUID, sprint_id: UUID, current_user: UserModel = Depends(get_current_user)):
    ...
    try:
        from app.services.events import event_bus, EventType
        event_bus.emit(EventType.SPRINT_COMPLETED, {
            "sprint_id": str(sprint_id),
            "sprint_name": data[0].get("name", "Sprint"),
            "project_id": str(project_id),
            "user_id": current_user.id,
