...
class InviteWorkspaceMemberResponse(WorkspaceMember):
...
class CreateWorkspaceInviteLinkRequest(BaseModel):
    """Create a shareable workspace invite link (token-based)."""
    role: str = Field("viewer", pattern="^(owner|admin|member|viewer)$")
    expires_days: int = Field(7, ge=1, le=30)


class CreateWorkspaceInviteLinkResponse(BaseModel):
    invite_link: str
...
@router.post("/{workspace_id}/members/invite", response_model=InviteWorkspaceMemberResponse, status_code=status.HTTP_201_CREATED)
def invite_workspace_member(workspace_id: UUID, body: InviteWorkspaceMember, current_user: UserModel = Depends(get_current_user)):
    ...
    Creates/keeps a pending workspace_members row (status=invited) and generates a
    token-based invitation in the invitations table so that clicking the email link
    can securely activate membership.
    """
    _require_workspace_member(str(workspace_id), str(current_user.id), ["owner", "admin"])
    email_clean = body.email.strip().lower()
    try:
        # Ensure a pending membership row exists (so admins can see pending invites)
        ...
        try:
            existing_res = (
                ...
                .select("id,workspace_id,user_id,invited_email,role,status,created_at,joined_at")
                .eq("workspace_id", str(workspace_id))
                .eq("invited_email", email_clean)
                .maybe_single()
                .execute()
            )
            existing_pending = getattr(existing_res, "data", None)
        except Exception:
            existing_pending = None

        if existing_pending and (existing_pending.get("status") in ("invited", "pending")):
            member_row = existing_pending
        else:
            insert_payload = {
        ...
        # Create invitation token record (required for accept flow)
        ...
        supabase.table("invitations").insert(invitation_payload).execute()

        frontend_url = os.getenv("FRONTEND_URL", "http://localhost:5173").rstrip("/")
        invite_link = f"{frontend_url}/accept-invite?token={invitation_token}&workspace={workspace_id}"
        ...
        if body.send_email:
            try:
                # Load workspace name for nicer email
                workspace_name = "a workspace"
                try:
                    ws_res = supabase.table("workspaces").select("name").eq("id", str(workspace_id)).maybe_single().execute()
                    ws_row = getattr(ws_res, "data", None)
                    if ws_row and ws_row.get("name"):
                        workspace_name = ws_row.get("name")
                except Exception:
                    pass
        ...
        return InviteWorkspaceMemberResponse(
            **WorkspaceMember(**member_row).dict(),
            invite_link=invite_link,
...
@router.post("/{workspace_id}/invite-link", response_model=CreateWorkspaceInviteLinkResponse)
def create_workspace_invite_link(
    ...
    This is token-based and does not require an invited email.
    """
    _require_workspace_member(str(workspace_id), str(current_user.id), ["owner", "admin"])
    try:
        invitation_token = str(uuid4())
        ...
        logger.error(f"Failed to create workspace invite link for {workspace_id}: {e}")
