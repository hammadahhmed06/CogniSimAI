...
try:
    ...
    def compute_quality_score(distinctness, criteria_density, warning_penalty, structure_valid, llm_alignment=None): return 0.0  # type: ignore
...
MODEL_NAME = getattr(epic_decomposer, "MODEL_NAME", "x-ai/grok-4.1-fast")
...
def _recalculate_quality_score(
    ...
    warnings_per_story = len(warnings) / total if total else 0
    warning_penalty = 1 - min(1.0, (warnings_per_story / 5.0))
    dup_count = len(duplicate_matches or [])
...
async def epic_decompose(body: EpicDecomposeRequest, current_user: UserModel = Depends(get_current_user), ctx: TeamContext = Depends(get_team_context)):
    ...
    if not body.stories:
        ...
        input_token_estimate = estimate_tokens(prompt_source, model=MODEL_NAME)
        ...
        if result.get('success') and raw and isinstance(raw, dict) and isinstance(raw.get('stories'), list):
            ...
            analyst_insights = result.get('analyst_insights') or None
            review_feedback = result.get('review_feedback') or None
        else:
        ...
        if quality_score is None:
            ...
            if isinstance(review_feedback, dict):
                llm_alignment = review_feedback.get('llm_score')
            quality_score = _recalculate_quality_score(stories, warnings, duplicate_matches, llm_alignment=llm_alignment)
        joined = '\n'.join([s.title + ' ' + ' '.join(s.acceptance_criteria) for s in stories])
        output_token_estimate = estimate_tokens(joined, model=MODEL_NAME)
    else:
        if not body.stories:
        ...
        if stories:
            ...
            quality_score = _recalculate_quality_score(stories, warnings)
    ...
    if run_id:
        ...
        if quality_score is None:
            ...
            if isinstance(review_feedback, dict):
                llm_alignment = review_feedback.get('llm_score')
            quality_score = _recalculate_quality_score(stories, warnings, llm_alignment=llm_alignment)
    ...
    total_token_estimate = (input_token_estimate or 0) + (output_token_estimate or 0)
    total_token_estimate = total_token_estimate or None
...
@router.post("/epic/decompose/stream")
async def epic_decompose_stream(
    body: EpicDecomposeRequest,
    current_user: UserModel = Depends(get_current_user),
    ctx: TeamContext = Depends(get_team_context)
):
    epic, epic_uuid = _validate_and_fetch_epic(body.epic_id, current_user.id)

    # Enforce per-team daily run limit
    from datetime import timedelta
    now_utc = datetime.now(timezone.utc)
    day_start = now_utc.replace(hour=0, minute=0, second=0, microsecond=0)
    day_end = day_start + timedelta(days=1)
    ...
    async def event_generator():
        ...
        async for event in epic_decomposer.decompose_epic_stream(
            epic_description=prompt_source or 'Epic',
            max_stories=body.max_stories,
            epic_id=str(epic['id']),
            user_prompt=body.user_prompt or None,
            ...
            if event["type"] == "result":
                ...
                # Persist items BEFORE yielding result so client can fetch them immediately
                if run_id and final_result:
                    stories = final_result.get("data", {}).get("stories", [])
                    if stories:
                        try:
                            story_objs = [GeneratedStory(**s) for s in stories]
                            _persist_run_items(run_id, story_objs)
                        except Exception as e:
                            print(f"Failed to persist run items: {e}")
        ...
        if run_id and final_result:
            ...
            input_tokens = estimate_tokens(prompt_source, model=MODEL_NAME)
            output_text = json.dumps(stories)
            output_tokens = estimate_tokens(output_text, model=MODEL_NAME)
            total_tokens = input_tokens + output_tokens
            cost_estimate = round(total_tokens * 0.000002, 6)
            
            _update_run(run_id, AgentRunUpdate(
                status="succeeded" if final_result.get("success") else "failed",
                output={
                    "stories": stories,
                    "warnings": warnings,
                    "model": MODEL_NAME,
                    "quality_score": final_result.get("quality_score"),
                    "warnings_count": final_result.get("warnings_count"),
                    "duplicate_matches": final_result.get("duplicate_matches"),
                    "analyst_insights": final_result.get("analyst_insights"),
...
# ============= FEATURE REQUEST / FEEDBACK SUBMISSION =============

class FeatureRequestCreate(BaseModel):
    """Model for submitting feature requests or feedback"""
    category: str  # 'agent_request', 'bug_report', 'improvement', 'general'
    title: str
    description: str
    priority: Optional[str] = 'medium'  # 'low', 'medium', 'high'
    contact_preference: Optional[str] = None  # 'email', 'none'


class JiraEpicDecomposeRequest(BaseModel):
    """Request to decompose an epic from Jira and optionally push stories back"""
    epic_key: str = Field(..., description="Jira epic key (e.g., 'PROJ-123')")
    integration_id: str = Field(..., description="Integration credentials ID")
    workspace_id: Optional[str] = Field(None, description="Workspace ID owning the integration (falls back to team_id)")
    max_stories: int = Field(6, ge=3, le=12, description="Maximum stories to generate")
    user_prompt: Optional[str] = Field(None, description="Optional user guidance")
    push_to_jira: bool = Field(False, description="Whether to push stories back to Jira")


@router.post("/epic/decompose/jira")
async def epic_decompose_from_jira(
    body: JiraEpicDecomposeRequest,
    current_user: UserModel = Depends(get_current_user),
    ctx: TeamContext = Depends(get_team_context)
):
    """
    Decompose an epic fetched directly from Jira.
    Optionally pushes generated stories back to Jira as linked stories.
    
    This endpoint:
    1. Fetches the epic from Jira using the provided integration
    2. Decomposes it into user stories using AI
    3. Optionally creates the stories in Jira with epic link
    """
    from app.agents.epic_decomposer import decompose_epic_from_jira
    ...
    try:
        result = await decompose_epic_from_jira(
            epic_key=body.epic_key,
            integration_id=body.integration_id,
            workspace_id=body.workspace_id or str(ctx.team_id),
            max_stories=body.max_stories,
            user_prompt=body.user_prompt,
            push_to_jira=body.push_to_jira
        )
        ...
        return {
            ...
            "jira_push_results": result.get("jira_push_results"),
            "provider": result.get("provider"),
            "error": result.get("error")
        }
        
    except Exception as e:
        logger.error(f"Jira epic decomposition failed: {e}")
...
@router.post("/feedback", response_model=FeatureRequestResponse)
async def submit_feedback(
    ...
    """
    Submit feedback, feature requests, or agent requests.
    Categories:
    - 'agent_request': Request a new AI agent
    ...
    # Validate category
    valid_categories = {'agent_request', 'bug_report', 'improvement', 'general'}
    ...
    try:
        result = supabase.table("feature_requests").insert(feedback_data).execute()
        ...
        # Fallback: try storing in a simpler way if table doesn't exist
        logger.warning(f"Failed to save feedback to feature_requests table: {e}")
        # Return a mock response since the feedback was received
        return FeatureRequestResponse(
            id=feedback_id,
            category=body.category,
            title=body.title.strip()[:200],
