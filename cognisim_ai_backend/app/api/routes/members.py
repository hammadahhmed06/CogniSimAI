...
def _load_users_table_identities(user_ids: List[str]) -> dict[str, dict]:
    """Preferred identity source: public.users.

    The teams module syncs Supabase auth users into the `users` table with
    at least: id, email, full_name, avatar_url. Using it here avoids showing
    raw UUIDs when user_profiles is missing.
    """
    if not user_ids:
        return {}
    try:
        res = (
            ...
            .in_("id", [str(u) for u in user_ids])
            .execute()
        )
...
async def list_members(
    ...
    # Fetch identities. Prefer public.users (synced from auth.users); fallback to user_profiles.
    users_map = _load_users_table_identities(user_ids)
    ...
    for uid in user_ids:
        ...
        if q:
            t = (
                (prof.get("full_name") or ident.get("full_name") or "")
                + " "
                + (prof.get("title") or "")
                + " "
                + (ident.get("email") or prof.get("email") or "")
            )
        ...
        result.append(MemberProfile(
            user_id=UUID(uid),
            email=ident.get("email") or prof.get("email"),
            full_name=prof.get("full_name") or ident.get("full_name"),
            title=prof.get("title"),
            bio=prof.get("bio"),
            timezone=prof.get("timezone"),
            location=prof.get("location"),
            avatar_url=prof.get("avatar_url") or ident.get("avatar_url"),
    ...
    if sort_key == "availability":
        ...
        result.sort(key=lambda m: ((m.full_name or m.email or "").lower()))
    total = len(result)
...
async def get_member(user_id: UUID, current_user: UserModel = Depends(get_current_user), wctx: WorkspaceContext = Depends(get_workspace_context)):
    ...
    ident_map = _load_users_table_identities([str(user_id)])
    ident = ident_map.get(str(user_id), {})
    prof_res = (
